...
@pytest.fixture
def mock_publish():
    """Mock publish_event to avoid hitting Pub/Sub. GOOGLE_CLOUD_PROJECT is set in these tests."""
    with patch("src.input.pubsub.publish_event", return_value="test-message-id") as m_pub, \
         patch("src.input.webhooks.acc.publish_event", return_value="test-message-id") as m_a:
        yield {"fireflies": m_pub, "acc": m_a}
