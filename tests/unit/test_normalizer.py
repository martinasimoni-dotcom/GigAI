...
def test_normalize_event_retries_on_json_decode_error(monkeypatch):
    """JSONDecodeError triggers up to 3 retries before raising."""
    from src.system.data_processing import normalizer
    ...
    assert call_count["n"] == 3  # 3 attempts via tenacity


def test_normalize_event_no_retry_on_validation_error(monkeypatch):
    """Pydantic ValidationError is NOT retried — fails immediately."""
    from src.system.data_processing import normalizer
    ...
    with patch.object(normalizer, "call_haiku", side_effect=_haiku_side_effect):
        with pytest.raises(Exception):
            normalizer.normalize_event(raw)
    assert call_count["n"] == 1  # no retry on validation errors
